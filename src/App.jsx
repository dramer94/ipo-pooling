...
export default function IPOPoolManager() {
  ...
  const calculateDistribution = () => {
    ...
    // Calculate tier matching transfers (original fund transfers)
    distribution = distribution.map((p) => {
    ...
    // Calculate settlement transfers (profit distribution)
    // Those who got allocation have the actual profit and need to distribute
    distribution = distribution.map((p) => {
      if (p.gotAllocation) {
        // They received actual profit, need to give away the difference
        ...
        // They didn't get allocation but deserve profit share
        const settlementTransferIn = p.profitShare;
        return { ...p, settlementTransferIn };
      }
    });

    // Calculate net position: what they keep after all transfers
    distribution = distribution.map((p) => {
      const netPosition =
                ...
                <div className="bg-blue-50 border-l-4 border-blue-500 p-4 mb-4">
                  <div className="flex items-start gap-2">
                    ...
                    <div className="text-sm text-blue-800">
                      ...
                      <p>3. Enter lots applied for each applicant</p>
                      <p>
                    ...
                    <div className="bg-purple-50 border-l-4 border-purple-500 p-4 mb-6">
                      ...
                      <div className="text-sm text-purple-800 grid grid-cols-1 md:grid-cols-2 gap-2">
                        ...
                        <div>
                          ...
                          <p>
                            • <strong>ROI (vs Initial Capital):</strong> Return
                            on investment based on your total capital
                            contribution
                          </p>
                          <p>
                            • <strong>ROI (vs Allocated Capital):</strong>{" "}
                            Return on investment based on capital actually used
                            for IPO application
                          </p>
                          <p className="mt-2 text-xs italic">
                            Note: Selling fees are deducted before profit is
                            ...
                            if (onlyCapitalProviders.length === 0) {
                              ...
                              formulaBreakdown = `(${p.capitalPercent.toFixed(
                                ...
                                bonus > 0
                                  ...
                                  : ""
                              } = RM ${mainPoolShare.toFixed(2)}${
                                bonus > 0 ? ` + RM ${bonus.toFixed(2)}` : ""
                              }`;
                            } else {
                              // Scenario 2: Some only provide capital
                              ...
                              formulaBreakdown = `(${p.capitalPercent.toFixed(
                                ...
                                applierBonus > 0
                                  ...
                                  : ""
                              } = RM ${capitalPoolShare.toFixed(2)}${
                                applierBonus > 0
                                  ? ` + RM ${applierBonus.toFixed(2)}`
                                  : ""
                              }`;
                            }
